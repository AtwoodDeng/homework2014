...
byte[] buffersSend = new byte[width*height*4];
byte[] buffersRecv = new byte[width*height*4];
int recvCount = 0;
...
void draw() {
  ...
  if (video.available()) {
    ...
    arraycopy( backgroundPixels , pixels);
...
void get_buffer() {
  if(c.available() > 0)
  {
    int tempLength = width * height * 4 - recvCount ;
    ...
    //println( "res back " + backgroundPixels[i] + "bufsend " + buffersRecv[ i * 4 + 3 ] + " " 
    // + buffersRecv[ i * 4 + 2 ] + " " + buffersRecv[ i * 4 + 1 ]
    //+ " " + buffersRecv[ i * 4 + 0 ] );
    
    //if( backgroundPixels[i] == 0xFF0000FF )
    //  println("haha" );
    //  
     }
    }
...
void setFrontPixels(){
  ...
  for ( int i = 0 ; i < height ;  ++ i ) {
     for ( int j = 0 ; j < width ; ++ j ) {
       ...
       {
          frontPixels[ i * width + j ] = 0xFFFAFF72;
       }
       else
       {
          frontPixels[ i * width + j ] = 0xFFAFDFE4;
       }
     }
   }
}
...
color getFrontPixel( int i , int j ) {
 ...
 void fliterSmallBlock( int[] input , int width , int height,  int num ) {
   for ( int i = 0 ; i < height ;  ++ i ) {
     for ( int j = 0 ; j < width ; ++ j ) {
       if ( input[i * width + j ] == -1 ) {
         if ( numSamePixel( input , i , j , width , height , -1 , num ) < num ) {
           if ( numSamePixel( input , i , j , width , height , -1 , num ) != 0 ) {
             print( "before " + numSamePixel( input , i , j , width , height , -1 , num ));
             clean( input , i , j , width , height , -1 , 1 );
             println( " after " + numSamePixel( input , i , j , width , height , -1 , num ));
           }
         }else{
           if ( numSamePixel( input , i , j , width , height , -1 , num ) != 0 ) {
             print( "before " + numSamePixel( input , i , j , width , height , -1 , num ));
             clean( input , i , j , width , height , -1 , 0 );
             println( " after " + numSamePixel( input , i , j , width , height , -1 , num ));
           }
         }
       }
 ...
 int numSamePixel( int[] input , int i , int j , int width , int height , int flag , int max ) {
     int total = 0 ;
     int ii = i;
     int jj = j;
     while( true ) {
        //find the most right point and most left point that equals flag
        int left = jj;
        int right = jj;
        //find the most left point
        while( left >= 0 && input[ ii * width + left ] == flag )
          left--;
        //find the most right point 
        while( right < width && input[ ii * width + right] == flag )
          right++;
        total += right - left;
        if ( total > max ) 
          break;
 ...
 void clean( int input[] , int i , int j , int width , int height ,int fromFlag, int toFlag) {
     int ii = i;
     int jj = j;
   while( true ) {
        //find the most right point and most left point that equals flag
        ...
        int right = jj;
        //find the most left point
        while( left >= 0 && input[ ii * width + left ] == fromFlag ) {
          input[ ii * width + left ] = toFlag;
          left--;
        }
        //find the most right point 
        while( right < width && input[ ii * width + right] == fromFlag ) {
          input[ ii * width + right ] = toFlag;
          right++;
        }
        //next row
        ii++;
